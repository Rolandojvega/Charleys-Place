...
def complete():
    ...
    if request.method == "POST":
        ...
        #print(rating[0])
        #menu = db.execute("SELECT MAX(menu_ID) FROM menu_details")
        print(like)
        print(dislike)

        menu = db.execute("SELECT MAX(menu_ID) FROM menu_details")
        current= (menu[0]['MAX(menu_ID)'])
...
def home():
    ...
    if request.method == "GET":
        ...
        #db.execute("INSERT INTO feedback (dish_ID, menu_ID, like_dislike, text_feedback, user_ip, timestamp) VALUES (XXX,  FROM dishes WHERE ID IN (SELECT dish_ID FROM menu_details WHERE menu_ID IN(SELECT MAX(menu_ID) FROM menu_details))")
...
#VALUES (values)
        ...
        #ratinglist = request.form.getlist("thumbsup")
        #print(ratinglist[0])
